...
class Tree_node(Node):   # 嵌入在 SGF 游戏中的节点。
...
class _Root_tree_node(Tree_node):  # 博弈树根节点
...
class Sgf_game:  # smart game format 游戏树  负责对这些字符串解码，并返回成python元组
    ...
    node. 完整的博弈树使用 Tree_nodes 表示。返回 Tree_nodes 的各种方法将始终为同一节点返回相同的对象
    Instantiate with
